        ...
        <div class="card mb-4">
            ...
            <form action="{{ route('applications.select') }}" method="post" id="applicantsSelectForm">
                ...
                <div class="card-body">
                    ...
                    <input type="hidden" name="job_id" value="{{ $job->id }}" id="applicationsJobID">
                                        ...
                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="d-flex align-items-center mb-3">
                                                    ...
                                                    <div>
                                                        <h4 class="h5 mb-1">
                                                            <strong>{{ $item->applicant->first_name }}
                                                                {{ $item->applicant->last_name }}</strong>
                                                            @if ($item->status == 'selected')
                                                                <span
                                                                    class="status-badge selected-badge ms-2 translatable">Selected</span>
                                                            @else
                                                                <span
                                                                    class="status-badge pending-badge ms-2 translatable">Pending</span>
                                                            @endif
                                                        </h4>
                                                        ...
                                                        @if (!is_null($item->profile))
                                                            <p class="mb-1 text-muted">
                                                                ...
                                                                {{ $item->profile->level }} in
                                                                {{ $item->profile->specialization }} with
                                                                {{ $item->profile->years_of_experience }} years experience
                                                            </p>
                                                            @php
                                                ...
                                                <div class="mb-3">
                                                    <h6 class="fw-bold"><i class="fas fa-comment me-1"></i> Application
                                                        Details</h6>
                                                    <div class="ps-3">
                                                        <p class="mb-1"><strong>Reason for applying:</strong>
                                                            {{ $item->reason }}</p>
                                                        <p class="mb-0"><strong>Cover letter:</strong>
                                                            {{ Str::limit($item->cover_letter, 150) }}</p>
                                                        @if (strlen($item->cover_letter) > 150)
                                                            <a href="#" class="text-primary" data-bs-toggle="modal"
                                                                data-bs-target="#applicationDetailsModal"
                                                                data-id="{{ $item->id }}"
                                                                id="applicationDetailsToggle">Read more</a>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-4">
                                                <div class="d-flex flex-wrap mb-3">
                                                    <a href="{{ route('student.details', $item->applicant->id) }}"
                                                        class="btn btn-outline-primary action-btn" target="_blank">
                                                        <i class="fas fa-user me-1"></i> View Profile
                                                    </a>
                                                    <a href="{{ route('application.cvdownload', $item->id) }}"
                                                        class="btn btn-outline-secondary action-btn" target="_blank">
                                                    ...
                                                    @foreach ($files as $file)
                                                        <a href="{{ route('download.file', $file) }}"
                                                            class="btn btn-outline-info file-download-btn" target="_blank">
                                                            <i class="fas fa-file-download me-1"></i> Doc
                                                            {{ $loop->iteration }}
                                                        </a>
                                                    @endforeach
                                                </div>
                                                ...
                                                <div class="form-check form-switch d-flex justify-content-end">
                                                    <input class="form-check-input applicantSelectToggle" type="checkbox"
                                                        role="switch" id="applicantSelectToggle{{ $item->id }}"
                                                        value="{{ $item->id }}"
                                                        {{ $item->status == 'selected' ? 'checked' : '' }}>
                                                    <label class="form-check-label ms-2"
                                                        for="applicantSelectToggle{{ $item->id }}">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                        <div class="col-md-4 form-group mb-3">
                                            ...
                                            <select name="interviewMethod" id="interviewMethod" class="form-control" required>
                                        ...
                                        <div class="col-md-4 form-group mb-3">
                                            <label for="interviewPlace" class="form-label">Interview Place</label>
                                            <input type="text" name="interviewPlace" id="interviewPlace"
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <textarea name="invitationLetter" id="invitationLetter" class="form-control" rows="5"
                                            placeholder="Write a personalized invitation message for selected candidates..."></textarea>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '#applicationDetailsToggle', function() {
                ...
                $.getJSON(`/application-details/${applicationid}`, function(data) {
                    ...
                    $('#modalTitleText').text(
                        `Cover Letter - ${data.applicant.first_name} ${data.applicant.last_name}`
                        );
                })
            });
        });
